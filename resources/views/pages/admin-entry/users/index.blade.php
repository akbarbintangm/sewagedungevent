...
@section('script')
	<script>
		function verifyUser(data) {
			var url = '{{ route('verifyUser:admin-entry', ['id' => ':data']) }}';
		...
		function unverifyUser(data) {
			var url = '{{ route('unverifyUser:admin-entry', ['id' => ':data']) }}';
			url = url.replace(':data', data);
		...
		function deleteUser(data) {
			var url = '{{ route('deleteUser:admin-entry', ['id' => ':data']) }}';
			url = url.replace(':data', data);
		...
		$(document).ready(function() {
			...
			var tableVerified = $('.table-user').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('listUser:admin-entry') }}",
				headers: {
					'CSRFToken': token
				},
				columns: [{
