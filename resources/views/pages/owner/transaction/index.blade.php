...
@section('script')
	<script>
		...
		function verifyOrder(data) {
			var url = '{{ route('updateOrder:owner', ['id' => ':data']) }}';
		...
		function cancelOrder(data) {
			var url = '{{ route('cancelOrder:owner', ['id' => ':data']) }}';
			url = url.replace(':data', data);
		...
		function verifyTransaction(data) {
			var url = '{{ route('updateTransaction:owner', ['id' => ':data']) }}';
			url = url.replace(':data', data);
		...
		async function getDetailTransaction(data) {
			...
			try {
				var url = '{{ route('detailTransaction:owner', ['id' => ':data']) }}';
				url = url.replace(':data', data);
		...
		$(document).ready(function() {
			...
			var tableVerified = $('.table-transaction').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('listTransaction:owner') }}",
				headers: {
					'CSRFToken': token
				},
				columns: [{
