							...
							@else
								@foreach ($data as $key => $item)
									<li class="list-group-item d-flex border-radius-lg mb-2 border-0 bg-gray-100 p-4">
										...
										<div class="ms-auto text-end">
											<a class="btn btn-link text-dark mb-0 px-3" href="{{ route('detailPageBuilding:owner', ['id' => $item->id]) }}"><i aria-hidden="true" class="fas fa-pencil-alt text-dark me-2"></i>Edit</a>
										</div>
									</li>
								@endforeach
							@endif
...
@section('script')
	<script>
		...
		async function getAllCountDashboard() {
			...
			try {
				var url = '{{ route('counterDashboard:owner') }}';
