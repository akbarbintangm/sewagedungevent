	...
	<script>
		$(document).ready(function() {
			...
			var tableVerified = $('.table-verified').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('listBuildingVerified:owner') }}",
				headers: {
					'CSRFToken': token
				},
				columns: [{
			...
			var tableUnverified = $('.table-unverified').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('listBuildingUnverified:owner') }}",
				headers: {
					'CSRFToken': token
				},
				columns: [{
			...
			var tableCanceled = $('.table-canceled').DataTable({
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('listBuildingCanceled:owner') }}",
				headers: {
					'CSRFToken': token
				},
				columns: [{
		...
		function deleteRoom(data) {
			var url = '{{ route('deleteBuilding:owner', ['id' => ':data']) }}';
		...
		function verifyRoom(data) {
			var url = '{{ route('verifyBuilding:owner', ['id' => ':data']) }}';
			url = url.replace(':data', data);
		...
		function cancelRoom(data) {
			var url = '{{ route('cancelBuilding:owner', ['id' => ':data']) }}';
			url = url.replace(':data', data);
